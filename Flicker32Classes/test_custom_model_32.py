...
MetadataCatalog.get("logo_test").set(thing_classes=classes)
...
cfg.MODEL.ROI_HEADS.NUM_CLASSES = len(classes)  # only has 32 classes. (see https://detectron2.readthedocs.io/tutorials/datasets.html#update-the-config-for-new-datasets)
...
#     )
#     out = v.draw_instance_predictions(outputs["instances"].to("cpu"))
#     cv2.imwrite("guess2/%s.jpg" % str(d["file_name"][-8:-5]), out.get_image()[:, :, ::-1])
